...
public class JasperReportsUtilsTests {
	...
	public void renderAsCsvWithDataSource() throws Exception {
		...
		JasperReportsUtils.renderAsCsv(getReport(), getParameters(), getDataSource(),
				writer);
	...
	public void renderAsHtmlWithDataSource() throws Exception {
		StringWriter writer = new StringWriter();
		JasperReportsUtils.renderAsHtml(getReport(), getParameters(), getDataSource(),
				writer);
		String output = writer.getBuffer().toString();
	...
	public void renderAsPdfWithExporterParameters() throws Exception {
		ByteArrayOutputStream os = new ByteArrayOutputStream();
		Map<JRExporterParameter, Object> exporterParameters = new HashMap<JRExporterParameter, Object>();
		exporterParameters.put(JRPdfExporterParameter.PDF_VERSION,
				JRPdfExporterParameter.PDF_VERSION_1_6.toString());
		JasperReportsUtils.renderAsPdf(getReport(), getParameters(), getData(), os,
				exporterParameters);
		byte[] output = os.toByteArray();
		assertPdfOutputCorrect(output);
	...
	public void renderWithWriter() throws Exception {
		StringWriter writer = new StringWriter();
		JasperPrint print = JasperFillManager.fillReport(getReport(), getParameters(),
				getDataSource());
	...
	private void assertCsvOutputCorrect(String output) {
		...
		assertTrue("Output should start with Dear Lord!",
	...
	private void assertXlsOutputCorrect(byte[] output) throws Exception {
		...
		assertEquals("Cell content should be Dear Lord!", "Dear Lord!",
				cell.getRichStringCellValue().getString());
	}

	private JasperReport getReport() throws Exception {
		ClassPathResource resource = new ClassPathResource("DataSourceReport.jasper",
				getClass());
	...
	private Map<String, Object> getParameters() {
		...
		model.put(JRParameter.REPORT_RESOURCE_BUNDLE, ResourceBundle.getBundle(
				"org/springframework/ui/jasperreports/messages", Locale.GERMAN));
		return model;
	}
