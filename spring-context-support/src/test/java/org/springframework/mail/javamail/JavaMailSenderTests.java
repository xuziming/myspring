...
public class JavaMailSenderTests {
	...
	public void javaMailSenderWithSimpleMessage() throws MessagingException, IOException {
		...
		List<Address> tos = Arrays.asList(
				sentMessage.getRecipients(Message.RecipientType.TO));
		assertEquals(1, tos.size());
		assertEquals("you@mail.org", ((InternetAddress) tos.get(0)).getAddress());
		List<Address> ccs = Arrays.asList(
				sentMessage.getRecipients(Message.RecipientType.CC));
		...
		List<Address> bccs = Arrays.asList(
				sentMessage.getRecipients(Message.RecipientType.BCC));
	...
	public void testJavaMailSenderWithSimpleMessages()
		...
		List<Address> tos1 = Arrays.asList(
				sentMessage1.getRecipients(Message.RecipientType.TO));
		...
		List<Address> tos2 = Arrays.asList(
				sentMessage2.getRecipients(Message.RecipientType.TO));
	...
	public void testJavaMailSenderWithMimeMessage() throws MessagingException {
		...
		mimeMessage.setRecipient(Message.RecipientType.TO,
				new InternetAddress("you@mail.org"));
	...
	public void javaMailSenderWithMimeMessages() throws MessagingException {
		...
		mimeMessage1.setRecipient(Message.RecipientType.TO,
				new InternetAddress("he@mail.org"));
		MimeMessage mimeMessage2 = sender.createMimeMessage();
		mimeMessage2.setRecipient(Message.RecipientType.TO,
				new InternetAddress("she@mail.org"));
	...
	@Test
	public void javaMailSenderWithMimeMessageHelperAndSpecificEncoding()
	...
	@Test
	public void javaMailSenderWithMimeMessageHelperAndDefaultEncoding()
			throws MessagingException {
		MockJavaMailSender sender = new MockJavaMailSender();
		sender.setHost("host");
		sender.setUsername("username");
	...
	public void javaMailSenderWithCustomSession() throws MessagingException {
		...
		MockJavaMailSender sender = new MockJavaMailSender() {
			...
			@Override
			protected Transport getTransport(Session sess)
	...
	public void failedSimpleMessage() throws Exception {
		...
		try {
			...
			assertTrue(sender.transport.isCloseCalled());
			assertEquals(1, sender.transport.getSentMessages().size());
			assertEquals(new InternetAddress("she@mail.org"),
					sender.transport.getSentMessage(0).getAllRecipients()[0]);
			assertEquals(1, ex.getFailedMessages().size());
			assertEquals(simpleMessage1,
					ex.getFailedMessages().keySet().iterator().next());
	...
	private static class MockTransport extends Transport {
		...
		public void sendMessage(Message message, Address[] addresses)
			...
			if (message.getSubject() != null && message.getSubject().contains("custom")) {
				assertEquals(new GregorianCalendar(2005, 3, 1).getTime(),
						message.getSentDate());
			}
