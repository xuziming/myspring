...
public abstract class AbstractJCacheAnnotationTests {
	...
	public void cacheExceptionRewriteCallStack() {
		...
		assertEquals("First call should not have been cached", ref + 1,
				service.exceptionInvocations());
		...
		assertEquals("Second call should have been cached", ref + 1,
				service.exceptionInvocations());
		...
		assertFalse("Original stack must not contain any reference to methodInCallStack",
				contain(first, AbstractJCacheAnnotationTests.class.getName(),
						"methodInCallStack"));
		assertTrue(
				"Cached stack should have been rewritten with a reference to  methodInCallStack",
				contain(second, AbstractJCacheAnnotationTests.class.getName(),
						"methodInCallStack"));
	}
	...
	/**
	 * The only purpose of this method is to invoke a particular method on the service so
	 * that the call stack is different.
	 */
