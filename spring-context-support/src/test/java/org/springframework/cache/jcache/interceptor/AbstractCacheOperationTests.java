...
public abstract class AbstractCacheOperationTests<O extends JCacheOperation<?>>
	...
	public void simple() {
		...
		assertEquals("Unexpected number of annotation on " + operation.getMethod(), 1,
	...
	protected void assertCacheInvocationParameter(CacheInvocationParameter actual,
			Class<?> targetType, Object value, int position) {
		assertEquals("wrong parameter type for " + actual, targetType,
				actual.getRawType());
		assertEquals("wrong parameter value for " + actual, value, actual.getValue());
		assertEquals("wrong parameter position for " + actual, position,
				actual.getParameterPosition());
	}
	...
	protected <A extends Annotation> CacheMethodDetails<A> create(Class<A> annotationType,
		...
		return new DefaultCacheMethodDetails<A>(method, cacheAnnotation,
				getCacheName(cacheAnnotation));
	}
