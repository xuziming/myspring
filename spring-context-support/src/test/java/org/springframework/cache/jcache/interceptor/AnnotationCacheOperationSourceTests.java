...
public class AnnotationCacheOperationSourceTests extends AbstractJCacheTests {
	...
	@Test
	public void cache() {
		CacheResultOperation op = getDefaultCacheOperation(CacheResultOperation.class,
				String.class);
		assertDefaults(op);
		assertNull("Exception caching not enabled so resolver should not be set",
	...
	@Test
	public void put() {
		CachePutOperation op = getDefaultCacheOperation(CachePutOperation.class,
	...
	@Test
	public void remove() {
		CacheRemoveOperation op = getDefaultCacheOperation(CacheRemoveOperation.class,
				String.class);
		assertDefaults(op);
	}
	...
	public void removeAll() {
		CacheRemoveAllOperation op = getDefaultCacheOperation(
				CacheRemoveAllOperation.class);
		assertEquals(defaultCacheResolver, op.getCacheResolver());
	}

	@Test
	public void noAnnotation() {
		assertNull(getCacheOperation(AnnotatedJCacheableService.class,
				name.getMethodName()));
	}
	...
	public void defaultCacheNameWithDefaults() {
		...
		assertEquals("java.lang.Object.toString()",
				source.determineCacheName(method, mock, ""));
	}
	...
	public void defaultCacheNameWithParameters() {
		Method method = ReflectionUtils.findMethod(Comparator.class, "compare",
	...
	public void customCacheResolver() {
		CacheResultOperation operation = getCacheOperation(CacheResultOperation.class,
				CustomService.class, name.getMethodName(), Long.class);
		...
		assertEquals(defaultKeyGenerator,
	...
	public void customKeyGenerator() {
		...
		assertCacheKeyGenerator(operation.getKeyGenerator(),
				TestableCacheKeyGenerator.class);
	}
	...
	public void customKeyGeneratorAndCacheResolverWithExceptionName() {
		CacheResultOperation operation = getCacheOperation(CacheResultOperation.class,
				CustomServiceWithDefaults.class, name.getMethodName(), Long.class);
		assertJCacheResolver(operation.getCacheResolver(), TestableCacheResolver.class);
		assertJCacheResolver(operation.getExceptionCacheResolver(),
				TestableCacheResolver.class);
		assertCacheKeyGenerator(operation.getKeyGenerator(),
				TestableCacheKeyGenerator.class);
	}
	...
	protected <T extends JCacheOperation<?>> T getDefaultCacheOperation(
		...
		return getCacheOperation(operationType, AnnotatedJCacheableService.class,
				name.getMethodName(), parameterTypes);
	}

	protected <T extends JCacheOperation<?>> T getCacheOperation(Class<T> operationType,
			Class<?> targetType, String methodName, Class<?>... parameterTypes) {

		JCacheOperation<?> result = getCacheOperation(targetType, methodName,
				parameterTypes);
	...
	private JCacheOperation<?> getCacheOperation(Class<?> targetType, String methodName,
			Class<?>... parameterTypes) {
		Method method = ReflectionUtils.findMethod(targetType, methodName,
				parameterTypes);
	...
	private void assertJCacheResolver(CacheResolver actual,
		...
		if (expectedTargetType == null) {
			...
			assertEquals("Wrong cache resolver implementation",
					CacheResolverAdapter.class, actual.getClass());
			CacheResolverAdapter adapter = (CacheResolverAdapter) actual;
			assertEquals("Wrong target JCache implementation", expectedTargetType,
	...
	private void assertCacheKeyGenerator(KeyGenerator actual,
		...
		assertEquals("Wrong cache resolver implementation", KeyGeneratorAdapter.class,
				actual.getClass());
		KeyGeneratorAdapter adapter = (KeyGeneratorAdapter) actual;
		assertEquals("Wrong target CacheKeyGenerator implementation", expectedTargetType,
				adapter.getTarget().getClass());
	}
