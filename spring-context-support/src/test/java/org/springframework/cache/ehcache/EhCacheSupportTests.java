...
public class EhCacheSupportTests {
	...
	public void testBlankCacheManager() throws Exception {
		...
		try {
			...
			assertTrue("Loaded CacheManager with no caches",
					cm.getCacheNames().length == 0);
	...
	public void testCacheManagerFromConfigFile() throws Exception {
		EhCacheManagerFactoryBean cacheManagerFb = new EhCacheManagerFactoryBean();
		cacheManagerFb.setConfigLocation(
				new ClassPathResource("testEhcache.xml", getClass()));
		cacheManagerFb.setCacheManagerName("myCacheManager");
		cacheManagerFb.afterPropertiesSet();
		try {
			CacheManager cm = cacheManagerFb.getObject();
			...
			Cache myCache1 = cm.getCache("myCache1");
			assertFalse("myCache1 is not eternal",
					myCache1.getCacheConfiguration().isEternal());
			assertTrue("myCache1.maxElements == 300",
					myCache1.getCacheConfiguration().getMaxElementsInMemory() == 300);
		}
		finally {
			cacheManagerFb.destroy();
		}
	...
	private void doTestEhCacheFactoryBean(boolean useCacheManagerFb) throws Exception {
		...
		try {
			...
			assertTrue("default maxElements is correct",
					config.getMaxElementsInMemory() == 10000);
			assertFalse("default eternal is correct", config.isEternal());
			assertTrue("default timeToLive is correct",
					config.getTimeToLiveSeconds() == 120);
			assertTrue("default timeToIdle is correct",
					config.getTimeToIdleSeconds() == 120);
			assertTrue("default diskExpiryThreadIntervalSeconds is correct",
					config.getDiskExpiryThreadIntervalSeconds() == 120);
			...
			assertTrue("overridden maxElements is correct",
					config.getMaxElementsInMemory() == 5);
			assertTrue("default timeToLive is correct",
					config.getTimeToLiveSeconds() == 8);
			assertTrue("default timeToIdle is correct",
					config.getTimeToIdleSeconds() == 7);
			assertTrue("overridden diskExpiryThreadIntervalSeconds is correct",
					config.getDiskExpiryThreadIntervalSeconds() == 10);
		}
		finally {
