...
public class QuartzSupportTests {
	...
	public void schedulerWithTaskExecutor() throws Exception {
		...
		Thread.sleep(500);
		assertTrue("DummyJob should have been executed at least once.",
				DummyJob.count > 0);
	...
	@Test
	public void schedulerWithSpringBeanJobFactoryAndParamMismatchNotIgnored()
			throws Exception {
		Assume.group(TestGroup.PERFORMANCE);
	...
	public void schedulerWithSpringBeanJobFactoryAndJobSchedulingData() throws Exception {
		...
		SchedulerFactoryBean bean = new SchedulerFactoryBean();
		bean.setJobFactory(new SpringBeanJobFactory());
		bean.setJobSchedulingDataLocation(
				"org/springframework/scheduling/quartz/job-scheduling-data.xml");
		bean.afterPropertiesSet();
		bean.start();
	...
	public void twoAnonymousMethodInvokingJobDetailFactoryBeans() throws Exception {
		Assume.group(TestGroup.PERFORMANCE);
		ClassPathXmlApplicationContext ctx = context(
		...
		try {
			...
			assertEquals("doImport called exportService", 0,
					exportService.getImportCount());
			assertEquals("doExport not called on exportService", 2,
					exportService.getExportCount());
			assertEquals("doImport not called on importService", 2,
					importService.getImportCount());
			assertEquals("doExport called on importService", 0,
	...
	public void schedulerAutoStartsOnContextRefreshedEventByDefault() throws Exception {
		...
		context.registerBeanDefinition("scheduler",
				new RootBeanDefinition(SchedulerFactoryBean.class));
	...
	public void schedulerAutoStartupFalse() throws Exception {
		...
		BeanDefinition beanDefinition = BeanDefinitionBuilder.genericBeanDefinition(
				SchedulerFactoryBean.class).addPropertyValue("autoStartup",
						false).getBeanDefinition();
	...
	public void schedulerRepositoryExposure() throws Exception {
		...
		assertSame(SchedulerRepository.getInstance().lookup("myScheduler"),
				ctx.getBean("scheduler"));
		ctx.close();
	}

	/**
	 * SPR-6038: detect HSQL and stop illegal locks being taken. TODO: Against Quartz 2.2,
	 * this test's job doesn't actually execute anymore...
	 */
	@Test
	public void schedulerWithHsqlDataSource() throws Exception {
		...
		assertFalse("No triggers were persisted",
				jdbcTemplate.queryForList("SELECT * FROM qrtz_triggers").isEmpty());

		/*
		 * Thread.sleep(3000); try {
		 * assertTrue("DummyJob should have been executed at least once.", DummyJob.count
		 * > 0); } finally { ctx.close(); }
		 */
	}
	...
	public static class DummyJobBean extends QuartzJobBean {
		...
		@Override
		protected synchronized void executeInternal(
