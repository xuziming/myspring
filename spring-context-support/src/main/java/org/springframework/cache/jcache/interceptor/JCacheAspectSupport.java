...
/**
 * Base class for JSR-107 caching aspects, such as the {@link JCacheInterceptor} or an
 * AspectJ aspect.
 *
 * <p>
 * Use the Spring caching abstraction for cache-related operations. No JSR-107
 * {@link javax.cache.Cache} or {@link javax.cache.CacheManager} are required to process
 * standard JSR-107 cache annotations.
 *
 * <p>
 * The {@link JCacheOperationSource} is used for determining caching operations
 *
 * <p>
 * A cache aspect is serializable if its {@code JCacheOperationSource} is serializable.
 *
 ...
 */
public class JCacheAspectSupport extends AbstractCacheInvoker
	...
	public void afterPropertiesSet() {
		Assert.state(getCacheOperationSource() != null,
				"The 'cacheOperationSource' property is required: "
						+ "If there are no cacheable methods, then don't use a cache aspect.");
		Assert.state(getErrorHandler() != null,
				"The 'errorHandler' property is required");
		...
		this.cacheRemoveEntryInterceptor = new CacheRemoveEntryInterceptor(
				getErrorHandler());
	...
	protected Object execute(CacheOperationInvoker invoker, Object target, Method method,
		...
		if (this.initialized) {
			...
			JCacheOperation<?> operation = getCacheOperationSource().getCacheOperation(
					method, targetClass);
			if (operation != null) {
				CacheOperationInvocationContext<?> context = createCacheOperationInvocationContext(
						target, args, operation);
	...
	private Object execute(CacheOperationInvocationContext<?> context,
		...
		if (operation instanceof CacheResultOperation) {
			return this.cacheResultInterceptor.invoke(
					(CacheOperationInvocationContext<CacheResultOperation>) context,
					adapter);
		}
		else if (operation instanceof CachePutOperation) {
			return this.cachePutInterceptor.invoke(
					(CacheOperationInvocationContext<CachePutOperation>) context,
					adapter);
		}
			...
			return this.cacheRemoveAllInterceptor.invoke(
					(CacheOperationInvocationContext<CacheRemoveAllOperation>) context,
					adapter);
		}
		else {
	...
	/**
	 * Execute the underlying operation (typically in case of cache miss) and return the
	 * result of the invocation. If an exception occurs it will be wrapped in a
	 * {@code ThrowableWrapper}: the exception can be handled or modified but it
