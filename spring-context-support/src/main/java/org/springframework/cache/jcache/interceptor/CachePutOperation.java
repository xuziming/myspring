...
class CachePutOperation extends AbstractJCacheKeyOperation<CachePut> {
	...
	public CachePutOperation(CacheMethodDetails<CachePut> methodDetails,
		...
		this.exceptionTypeFilter = createExceptionTypeFilter(ann.cacheFor(),
				ann.noCacheFor());
		this.valueParameterDetail = initializeValueParameterDetail(
				methodDetails.getMethod(), this.allParameterDetails);
		if (this.valueParameterDetail == null) {
			throw new IllegalArgumentException(
					"No parameter annotated with @CacheValue was found for " + ""
	...
	public CacheInvocationParameter getValueParameter(Object... values) {
		...
		if (parameterPosition >= values.length) {
			throw new IllegalStateException(
					"Values mismatch, value parameter at position " + parameterPosition
							+ " cannot be matched against " + values.length
							+ " value(s)");
		}
		return this.valueParameterDetail.toCacheInvocationParameter(
		...
		for (CacheParameterDetail parameter : allParameters) {
			if (parameter.isValue()) {
				if (result == null) {
					...
					throw new IllegalArgumentException(
							"More than one @CacheValue found on " + method + "");
				}
			}
		}
		return result;
