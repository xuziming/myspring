...
class CachePutInterceptor
	...
	protected Object invoke(CacheOperationInvocationContext<CachePutOperation> context,
		...
		CacheKeyInvocationContext<CachePut> invocationContext = createCacheKeyInvocationContext(
				context);
		...
		try {
			...
			if (!earlyPut
					&& operation.getExceptionTypeFilter().match(original.getClass())) {
				cacheValue(context, value);
			}
			throw ex;
		}
	...
	protected void cacheValue(CacheOperationInvocationContext<CachePutOperation> context,
			Object value) {
