...
public class JCacheCache extends AbstractValueAdaptingCache {
	...
	public <T> T get(Object key, Callable<T> valueLoader) {
		try {
			return this.cache.invoke(key, new ValueLoaderEntryProcessor<T>(),
					valueLoader);
		}
		...
		public T process(MutableEntry<Object, Object> entry, Object... arguments)
			...
			if (entry.exists()) {
				...
				try {
					...
					throw new EntryProcessorException("Value loader '" + valueLoader
							+ "' failed " + "to compute  value for key '" + entry.getKey()
							+ "'", ex);
				}
