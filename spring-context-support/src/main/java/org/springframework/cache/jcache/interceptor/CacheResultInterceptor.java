...
class CacheResultInterceptor extends AbstractKeyCacheInterceptor<CacheResultOperation, CacheResult> {
	...
	protected Object invoke(CacheOperationInvocationContext<CacheResultOperation> context,
		...
		try {
			...
			cacheException(exceptionCache, operation.getExceptionTypeFilter(), cacheKey,
					original);
			throw ex;
		}
	}
	...
	protected void checkForCachedException(Cache exceptionCache, Object cacheKey) {
		...
		if (result != null) {
			throw rewriteCallStack((Throwable) result.get(), getClass().getName(),
					"invoke");
		}
	}

	protected void cacheException(Cache exceptionCache, ExceptionTypeFilter filter,
			Object cacheKey, Throwable ex) {
		if (exceptionCache == null) {
			return;
		}
	...
	private Cache resolveExceptionCache(
		...
		if (exceptionCacheResolver != null) {
			return extractFrom(
					context.getOperation().getExceptionCacheResolver().resolveCaches(
							context));
		}
		return null;
	}
	 ...
	 *         stack up to (included) the common ancestor specified by the
	 *         {@code className} and {@code methodName} arguments, followed by stack trace
	 *         elements of the specified {@code exception} after the common ancestor.
	 */
	private static CacheOperationInvoker.ThrowableWrapper rewriteCallStack(
		...
		StackTraceElement[] result = new StackTraceElement[cachedIndex + callStack.length
				- index];
	...
	private static <T extends Throwable> T cloneException(T exception) {
		try {
			return (T) SerializationUtils.deserialize(
					SerializationUtils.serialize(exception));
		}
		catch (Exception ex) {
	...
	private static int findCommonAncestorIndex(StackTraceElement[] callStack,
