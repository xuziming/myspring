...
public abstract class AbstractFallbackJCacheOperationSource
	...
	private final Map<MethodClassKey, Object> cache = new ConcurrentHashMap<MethodClassKey, Object>(
			1024);
	...
	public JCacheOperation<?> getCacheOperation(Method method, Class<?> targetClass) {
		...
		if (cached != null) {
			return (cached != NULL_CACHING_ATTRIBUTE ? (JCacheOperation<?>) cached
					: null);
		}
		else {
	...
	private JCacheOperation<?> computeCacheOperation(Method method,
			Class<?> targetClass) {
