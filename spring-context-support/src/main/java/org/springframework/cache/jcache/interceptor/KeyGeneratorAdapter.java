...
/**
 * Spring's {@link KeyGenerator} implementation that either delegates to a standard
 * JSR-107 {@link javax.cache.annotation.CacheKeyGenerator}, or wrap a standard
 * {@link KeyGenerator} so that only relevant parameters are handled.
 *
...
class KeyGeneratorAdapter implements KeyGenerator {
	...
	/**
	 * Create an instance with the given {@link KeyGenerator} so that
	 * {@link javax.cache.annotation.CacheKey} and
	 * {@link javax.cache.annotation.CacheValue} are handled according to the spec.
	 */
	public KeyGeneratorAdapter(JCacheOperationSource cacheOperationSource,
			KeyGenerator target) {
	 ...
	 * {@link javax.cache.annotation.CacheKeyGenerator}.
	 */
	public KeyGeneratorAdapter(JCacheOperationSource cacheOperationSource,
	...
	public Object generate(Object target, Method method, Object... params) {
		JCacheOperation<?> operation = this.cacheOperationSource.getCacheOperation(method,
				target.getClass());
		if (!(AbstractJCacheKeyOperation.class.isInstance(operation))) {
			throw new IllegalStateException(
					"Invalid operation, should be a key-based operation " + operation);
		}
		CacheKeyInvocationContext<?> invocationContext = createCacheKeyInvocationContext(
	...
	private CacheKeyInvocationContext<?> createCacheKeyInvocationContext(Object target,
		...
		return new DefaultCacheKeyInvocationContext<Annotation>(keyCacheOperation, target,
				params);
	}
