...
abstract class AbstractJCacheOperation<A extends Annotation>
	 ...
	 */
	protected AbstractJCacheOperation(CacheMethodDetails<A> methodDetails,
		...
		this.allParameterDetails = initializeAllParameterDetails(
	...
	public CacheInvocationParameter[] getAllParameters(Object... values) {
		if (this.allParameterDetails.size() != values.length) {
			throw new IllegalStateException(
					"Values mismatch, operation has " + this.allParameterDetails.size()
		...
		for (int i = 0; i < this.allParameterDetails.size(); i++) {
			result.add(this.allParameterDetails.get(i).toCacheInvocationParameter(
					values[i]));
		}
	...
	private static List<CacheParameterDetail> initializeAllParameterDetails(
			Method method) {
