 ...
 * directly applying the values to the underlying MimeMessage. Allows for defining a
 * character encoding for the entire message, automatically applied by all methods of this
 * helper class.
 *
 * <p>
 * Offers support for HTML text content, inline elements such as images, and typical mail
 * attachments. Also supports personal names that accompany mail addresses. Note that
 ...
 * 		message.addAttachment("myDocument.pdf",
 * 				new ClassPathResource("doc/myDocument.pdf"));
 * 	}
 * });
 * </pre>
 *
 ...
 * {@link org.springframework.mail.SimpleMailMessage}) on top of this helper, in order to
 * let message population code interact with a simple message or a MIME message through a
 * common interface.
 *
 * <p>
 * <b>Warning regarding multipart mails:</b> Simple MIME messages that just contain HTML
 * text but no inline elements or attachments will work on more or less any email client
 * that is capable of HTML rendering. However, inline elements and attachments are still a
 * major compatibility issue between email clients: It's virtually impossible to get
 * inline elements and attachments working across Microsoft Outlook, Lotus Notes and Mac
 * Mail. Consider choosing a specific multipart mode for your needs: The javadoc on the
...
public class MimeMessageHelper {
	...
	/**
	 * Constant indicating a multipart message with a single root multipart element of
	 * type "mixed". Texts, inline elements and attachements will all get added to that
	 * root element.
	 * <p>
	 * This was Spring 1.0's default behavior. It is known to work properly on Outlook.
	 * However, other mail clients tend to misinterpret inline elements as attachments
	 * and/or show attachments inline as well.
	 */
	public static final int MULTIPART_MODE_MIXED = 1;

	/**
	 * Constant indicating a multipart message with a single root multipart element of
	 * type "related". Texts, inline elements and attachements will all get added to that
	 * root element.
	 * <p>
	 * This was the default behavior from Spring 1.1 up to 1.2 final. This is the
	 * "Microsoft multipart mode", as natively sent by Outlook. It is known to work
	 * properly on Outlook, Outlook Express, Yahoo Mail, and to a large degree also on Mac
	 * Mail (with an additional attachment listed for an inline element, despite the
	 * inline element also shown inline). Does not work properly on Lotus Notes
	 * (attachments won't be shown there).
	 */
	public static final int MULTIPART_MODE_RELATED = 2;

	/**
	 * Constant indicating a multipart message with a root multipart element "mixed" plus
	 * a nested multipart element of type "related". Texts and inline elements will get
	 * added to the nested "related" element, while attachments will get added to the
	 * "mixed" root element.
	 * <p>
	 * This is the default since Spring 1.2.1. This is arguably the most correct MIME
	 * structure, according to the MIME spec: It is known to work properly on Outlook,
	 * Outlook Express, Yahoo Mail, and Lotus Notes. Does not work properly on Mac Mail.
	 * If you target Mac Mail or experience issues with specific mails on Outlook,
	 * consider using MULTIPART_MODE_RELATED instead.
	 */
	...
	/**
	 * Create a new MimeMessageHelper for the given MimeMessage, assuming a simple text
	 * message (no multipart content, i.e. no alternative texts and no inline elements or
	 * attachments).
	 * <p>
	 * The character encoding for the message will be taken from the passed-in MimeMessage
	 * object, if carried there. Else, JavaMail's default encoding will be used.
	 * 
	 ...
	 * <p>
	 * Consider using the MimeMessageHelper constructor that takes a multipartMode
	 * argument to choose a specific multipart mode other than
	 ...
	 * 
	 * @param mimeMessage MimeMessage to work on
	 * @param multipart whether to create a multipart message that supports alternative
	 *        texts, inline elements and attachments (corresponds to
	 *        MULTIPART_MODE_MIXED_RELATED)
	 ...
	 * 
	 * @param mimeMessage MimeMessage to work on
	 * @param multipartMode which kind of multipart message to create (MIXED, RELATED,
	 *        MIXED_RELATED, or NO)
	 * @throws MessagingException if multipart creation failed
	...
	/**
	 * Determine the MimeMultipart objects to use, which will be used to store attachments
	 * on the one hand and text(s) and inline elements on the other hand.
	 * <p>
	 * Texts and inline elements can either be stored in the root element itself
	 * (MULTIPART_MODE_MIXED, MULTIPART_MODE_RELATED) or in a nested element rather than
	 * the root element directly (MULTIPART_MODE_MIXED_RELATED).
	 * <p>
	 * By default, the root MimeMultipart element will be of type "mixed"
	 * (MULTIPART_MODE_MIXED) or "related" (MULTIPART_MODE_RELATED). The main multipart
	 * element will either be added as nested element of type "related"
	 * (MULTIPART_MODE_MIXED_RELATED) or be identical to the root element itself
	 * (MULTIPART_MODE_MIXED, MULTIPART_MODE_RELATED).
	 * 
	 * @param mimeMessage the MimeMessage object to add the root MimeMultipart object to
	 * @param multipartMode the multipart mode, as passed into the constructor (MIXED,
	 *        RELATED, MIXED_RELATED, or NO)
	 * @throws MessagingException if multipart creation failed
	...
	protected void createMimeMultiparts(MimeMessage mimeMessage, int multipartMode)
		...
		switch (multipartMode) {
			...
			case MULTIPART_MODE_MIXED_RELATED:
				MimeMultipart rootMixedMultipart = new MimeMultipart(
						MULTIPART_SUBTYPE_MIXED);
				mimeMessage.setContent(rootMixedMultipart);
				MimeMultipart nestedRelatedMultipart = new MimeMultipart(
						MULTIPART_SUBTYPE_RELATED);
	 ...
	 * 
	 * @param root the root MimeMultipart object, which attachments will be added to; or
	 *        {@code null} to indicate no multipart at all
	 * @param main the main MimeMultipart object, which text(s) and inline elements will
	 *        be added to (can be the same as the root multipart object, or an element
	...
	/**
	 * Return whether this helper is in multipart mode, i.e. whether it holds a multipart
	 * message.
	 * 
	...
	/**
	 * Return the root MIME "multipart/mixed" object, if any. Can be used to manually add
	 * attachments.
	 * <p>
	...
	/**
	 * Return the underlying MIME "multipart/related" object, if any. Can be used to
	 * manually add body parts, inline elements, etc.
	 * <p>
	 ...
	 * @return the default encoding associated with the MimeMessage, or {@code null} if
	 *         none found
	 */
	 ...
	 * 
	 * @param mimeMessage the passed-in MimeMessage
	 * @return the default FileTypeMap associated with the MimeMessage, or a default
	 *         ConfigurableMimeFileTypeMap if none found for the message
	...
	/**
	 * Set the Java Activation Framework {@code FileTypeMap} to use for determining the
	 * content type of inline content and attachments that get added to the message.
	 * <p>
	 * Default is the {@code FileTypeMap} that the underlying MimeMessage carries, if any,
	 * or the Activation Framework's default {@code FileTypeMap} instance else.
	 * 
	 * @see #addInline
	 * @see #addAttachment
	...
	public void setFileTypeMap(FileTypeMap fileTypeMap) {
		this.fileTypeMap = (fileTypeMap != null ? fileTypeMap
				: getDefaultFileTypeMap(getMimeMessage()));
	}
	...
	/**
	 * Set whether to validate all addresses which get passed to this helper. Default is
	 * "false".
	 * <p>
	 * Note that this is by default just available for JavaMail >= 1.3. You can override
	 * the default {@code validateAddress method} for validation on older JavaMail
	 * versions (or for custom validation).
	 * 
	 * @see #validateAddress
	 */
	...
	/**
	 * Validate the given mail address. Called by all of MimeMessageHelper's address
	 * setters and adders.
	 * <p>
	 * Default implementation invokes {@code InternetAddress.validate()}, provided that
	 * address validation is activated for the helper instance.
	 * <p>
	 * Note that this method will just work on JavaMail >= 1.3. You can override it for
	...
	/**
	 * Validate all given mail addresses. Default implementation simply delegates to
	 * validateAddress for each address.
	 * 
	...
	private InternetAddress parseAddress(String address) throws MessagingException {
		...
		try {
			return (getEncoding() != null
					? new InternetAddress(raw.getAddress(), raw.getPersonal(),
							getEncoding())
					: raw);
		}
		catch (UnsupportedEncodingException ex) {
			throw new MessagingException(
					"Failed to parse embedded personal name to correct encoding", ex);
		}
	}
	 ...
	 * 
	 * @param priority the priority value; typically between 1 (highest) and 5 (lowest)
	...
	/**
	 * Set the given text directly as content in non-multipart mode or as default body
	 * part in multipart mode. Always applies the default content type "text/plain".
	 * <p>
	 * <b>NOTE:</b> Invoke {@link #addInline} <i>after</i> {@code setText}; else, mail
	...
	/**
	 * Set the given text directly as content in non-multipart mode or as default body
	 * part in multipart mode. The "html" flag determines the content type to apply.
	 * <p>
	 * <b>NOTE:</b> Invoke {@link #addInline} <i>after</i> {@code setText}; else, mail
	 * readers might not be able to resolve inline references correctly.
	 * 
	 * @param text the text for the message
	 * @param html whether to apply content type "text/html" for an HTML mail, using
	 *        default content type ("text/plain") else
	 * @throws MessagingException in case of errors
	 */
	...
	/**
	 * Set the given plain text and HTML text as alternatives, offering both options to
	 * the email client. Requires multipart mode.
	 * <p>
	 * <b>NOTE:</b> Invoke {@link #addInline} <i>after</i> {@code setText}; else, mail
	 * readers might not be able to resolve inline references correctly.
	 * 
	...
	private void setHtmlTextToMimePart(MimePart mimePart, String text)
			throws MessagingException {
		if (getEncoding() != null) {
			mimePart.setContent(text,
					CONTENT_TYPE_HTML + CONTENT_TYPE_CHARSET_SUFFIX + getEncoding());
		}
		else {
	 ...
	 * <p>
	 * Note that the InputStream returned by the DataSource implementation needs to be a
	 * <i>fresh one on each call</i>, as JavaMail will invoke {@code getInputStream()}
	 * multiple times.
	 * <p>
	 * <b>NOTE:</b> Invoke {@code addInline} <i>after</i> {@link #setText}; else, mail
	 * readers might not be able to resolve inline references correctly.
	 * 
	 * @param contentId the content ID to use. Will end up as "Content-ID" header in the
	 *        body part, surrounded by angle brackets: e.g. "myId" -> "&lt;myId&gt;". Can
	 *        be referenced in HTML source via src="cid:myId" expressions.
	 * @param dataSource the {@code javax.activation.DataSource} to take the content from,
	 *        determining the InputStream and the content type
	 * @throws MessagingException in case of errors
	 ...
	 * the like)!
	 * <p>
	 * Note that the InputStream returned by the Resource implementation needs to be a
	 * <i>fresh one on each call</i>, as JavaMail will invoke {@code getInputStream()}
	 * multiple times.
	 * <p>
	 * <b>NOTE:</b> Invoke {@code addInline} <i>after</i> {@link #setText}; else, mail
	 ...
	 * {@code org.springframework.core.InputStreamResource}, and specifying the content
	 * type explicitly.
	 * <p>
	 * You can determine the content type for any given filename via a Java Activation
	 * Framework's FileTypeMap, for example the one held by this helper.
	 * <p>
	 * Note that the InputStream returned by the InputStreamSource implementation needs to
	 * be a <i>fresh one on each call</i>, as JavaMail will invoke
	 * {@code getInputStream()} multiple times.
	 * <p>
	 * <b>NOTE:</b> Invoke {@code addInline} <i>after</i> {@code setText}; else, mail
	 * readers might not be able to resolve inline references correctly.
	 * 
	 * @param contentId the content ID to use. Will end up as "Content-ID" header in the
	 *        body part, surrounded by angle brackets: e.g. "myId" -> "&lt;myId&gt;". Can
	 ...
	 * @see #addInline(String, javax.activation.DataSource)
	 */
	public void addInline(String contentId, InputStreamSource inputStreamSource,
			String contentType) throws MessagingException {

		Assert.notNull(inputStreamSource, "InputStreamSource must not be null");
		if (inputStreamSource instanceof Resource
				&& ((Resource) inputStreamSource).isOpen()) {
			throw new IllegalArgumentException(
	 ...
	 *        (the content type will be determined by this)
	 * @param dataSource the {@code javax.activation.DataSource} to take the content from,
	 *        determining the InputStream and the content type
	 * @throws MessagingException in case of errors
	 * @see #addAttachment(String, org.springframework.core.io.InputStreamSource)
	 ...
	 * <p>
	 * The content type will be determined by the given filename for the attachment. Thus,
	 * any content source will be fine, including temporary files with arbitrary
	 * filenames.
	 * <p>
	 * Note that the InputStream returned by the InputStreamSource implementation needs to
	 * be a <i>fresh one on each call</i>, as JavaMail will invoke
	 ...
	 */
	protected DataSource createDataSource(final InputStreamSource inputStreamSource,
