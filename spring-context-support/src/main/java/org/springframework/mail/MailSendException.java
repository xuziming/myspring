...
public class MailSendException extends MailException {
	 ...
	 * @param msg the detail message
	 * @param cause the root cause from the mail API in use
	 * @param failedMessages Map of failed messages as keys and thrown exceptions as
	 *        values
	 */
	public MailSendException(String msg, Throwable cause,
		...
		this.messageExceptions = failedMessages.values().toArray(
				new Exception[failedMessages.size()]);
	}
	...
	public final Exception[] getMessageExceptions() {
		return (this.messageExceptions != null ? this.messageExceptions
				: new Exception[0]);
	}
	...
	public String toString() {
		if (ObjectUtils.isEmpty(this.messageExceptions)) {
			...
			sb.append("; message exceptions (").append(
					this.messageExceptions.length).append(") are:");
			for (int i = 0; i < this.messageExceptions.length; i++) {
				Exception subEx = this.messageExceptions[i];
