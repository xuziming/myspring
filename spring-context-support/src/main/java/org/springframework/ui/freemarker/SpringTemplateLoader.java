...
public class SpringTemplateLoader implements TemplateLoader {
	...
	public SpringTemplateLoader(ResourceLoader resourceLoader,
		...
		if (logger.isInfoEnabled()) {
			logger.info("SpringTemplateLoader for FreeMarker: using resource loader ["
					+ this.resourceLoader + "] and template loader path ["
	...
	public long getLastModified(Object templateSource) {
		...
		try {
			...
			if (logger.isDebugEnabled()) {
				logger.debug(
						"Could not obtain last-modified timestamp for FreeMarker template in "
								+ resource + ": " + ex);
			}
			return -1;
		}
