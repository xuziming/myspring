...
/**
 * Velocity ResourceLoader adapter that loads via a Spring ResourceLoader. Used by
 * VelocityEngineFactory for any resource loader path that cannot be resolved to a
 * {@code java.io.File}.
 *
 * <p>
 * Note that this loader does not allow for modification detection: Use Velocity's default
 * FileResourceLoader for {@code java.io.File} resources.
 *
 * <p>
 * Expects "spring.resource.loader" and "spring.resource.loader.path" application
 * attributes in the Velocity runtime: the former of type
...
public class SpringResourceLoader extends ResourceLoader {
	...
	public void init(ExtendedProperties configuration) {
		this.resourceLoader = (org.springframework.core.io.ResourceLoader) this.rsvc.getApplicationAttribute(
				SPRING_RESOURCE_LOADER);
		String resourceLoaderPath = (String) this.rsvc.getApplicationAttribute(
		...
		this.resourceLoaderPaths = StringUtils.commaDelimitedListToStringArray(
				resourceLoaderPath);
		...
		if (logger.isInfoEnabled()) {
			logger.info("SpringResourceLoader for Velocity: using resource loader ["
					+ this.resourceLoader + "] and resource loader paths "
	...
	public InputStream getResourceStream(String source) throws ResourceNotFoundException {
		...
		for (String resourceLoaderPath : this.resourceLoaderPaths) {
			org.springframework.core.io.Resource resource = this.resourceLoader.getResource(
					resourceLoaderPath + source);
			try {
		...
		throw new ResourceNotFoundException("Could not find resource [" + source
				+ "] in Spring resource loader path");
	}
