...
/**
 * Abstract tests for AspectJAdvisorFactory. See subclasses for tests of concrete
 * factories.
 *
 * @author Rod Johnson
 * @author Chris Beams
...
public abstract class AbstractAspectJAdvisorFactoryTests {
	...
	public void testRejectsPerCflowBelowAspect() {
		try {
			getFixture().getAdvisors(new SingletonMetadataAwareAspectInstanceFactory(
					new PerCflowBelowAspect(), "someBean"));
	...
	public void testPerTargetAspect() throws SecurityException, NoSuchMethodException {
		...
		assertTrue(sia.getPointcut().getMethodMatcher().matches(
				TestBean.class.getMethod("getSpouse"), null));
		...
		assertTrue(
				maaif.getAspectMetadata().getPerClausePointcut().getMethodMatcher().matches(
						TestBean.class.getMethod("getSpouse"), null));
	...
	@Test
	public void testMultiplePerTargetAspects()
		...
		advisors.addAll(getFixture().getAdvisors(
				new SingletonMetadataAwareAspectInstanceFactory(aspect1, "someBean1")));
		PerTargetAspect aspect2 = new PerTargetAspect();
		aspect2.setOrder(5);
		advisors.addAll(getFixture().getAdvisors(
				new SingletonMetadataAwareAspectInstanceFactory(aspect2, "someBean2")));
	...
	@Test
	public void testMultiplePerTargetAspectsWithOrderAnnotation()
			throws SecurityException, NoSuchMethodException {
		TestBean target = new TestBean();
		int realAge = 65;
		target.setAge(realAge);
	...
	public void testPerThisAspect() throws SecurityException, NoSuchMethodException {
		...
		TestBean itb = (TestBean) createProxy(target,
				getFixture().getAdvisors(new SingletonMetadataAwareAspectInstanceFactory(
						new PerThisAspect(), "someBean")),
				TestBean.class);
		assertEquals("Around advice must NOT apply", realAge, itb.getAge());

		Advised advised = (Advised) itb;
		// Will be ExposeInvocationInterceptor, synthetic instantiation advisor, 2 method
		// advisors
		...
		assertTrue(imapa.getDeclaredPointcut().getMethodMatcher().matches(
				TestBean.class.getMethod("getAge"), null));
	...
	public void testPerTypeWithinAspect()
		...
		TestBean itb = (TestBean) createProxy(target, getFixture().getAdvisors(aif),
				TestBean.class);
		...
		TestBean itb2 = (TestBean) createProxy(target, getFixture().getAdvisors(aif),
				TestBean.class);
		assertEquals(1, aif.getInstantiationCount());
		assertEquals("Around advice be independent for second instance", 0,
				itb2.getAge());
	...
	public void testBindingWithSingleArg() {
		...
		ITestBean itb = (ITestBean) createProxy(target,
				getFixture().getAdvisors(new SingletonMetadataAwareAspectInstanceFactory(
						new BindingAspectWithSingleArg(), "someBean")),
				ITestBean.class);
		itb.setAge(10);
		assertEquals("Around advice must apply", 20, itb.getAge());
		assertEquals(20, target.getAge());
	...
	public void testIntroductionOnTargetExcludedByTypePattern() {
		...
		List<?> proxy = (List<?>) createProxy(target,
				AopUtils.findAdvisorsThatCanApply(getFixture().getAdvisors(
						new SingletonMetadataAwareAspectInstanceFactory(
								new MakeLockable(), "someBean")),
						List.class),
				CannotBeUnlocked.class);
	 ...
	 * 
	 * @Test public void testIntroductionBasedOnAnnotationMatch_Spr5307() {
	 * AnnotatedTarget target = new AnnotatedTargetImpl();
	 * 
	 * List<Advisor> advisors = getFixture().getAdvisors( new
	 * SingletonMetadataAwareAspectInstanceFactory(new
	 * MakeAnnotatedTypeModifiable(),"someBean")); Object proxy = createProxy(target,
	 * advisors, AnnotatedTarget.class); System.out.println(advisors.get(1));
	 * assertTrue(proxy instanceof Lockable); Lockable lockable = (Lockable)proxy;
	 * lockable.locked(); }
	 */

	// Why does this test fail? It hasn't been run before, so it maybe never
	// actually passed...
	...
	public void testIntroductionWithArgumentBinding() {
		...
		List<Advisor> advisors = getFixture().getAdvisors(
				new SingletonMetadataAwareAspectInstanceFactory(
						new MakeITestBeanModifiable(), "someBean"));
		advisors.addAll(
				getFixture().getAdvisors(new SingletonMetadataAwareAspectInstanceFactory(
						new MakeLockable(), "someBean")));

		Modifiable modifiable = (Modifiable) createProxy(target, advisors,
				ITestBean.class);
	...
	public void testAspectMethodThrowsExceptionLegalOnSignature() {
		...
		List<Advisor> advisors = getFixture().getAdvisors(
				new SingletonMetadataAwareAspectInstanceFactory(
						new ExceptionAspect(expectedException), "someBean"));
	...
	protected Object createProxy(Object target, List<Advisor> advisors,
			Class<?>... interfaces) {
	...
	public void testTwoAdvicesOnOneAspect() {
		...
		List<Advisor> advisors = getFixture().getAdvisors(
				new SingletonMetadataAwareAspectInstanceFactory(twoAdviceAspect,
						"someBean"));
	...
	public void testAfterAdviceTypes() throws Exception {
		...
		List<Advisor> advisors = getFixture().getAdvisors(
				new SingletonMetadataAwareAspectInstanceFactory(afterReturningAspect,
						"someBean"));
		...
		assertEquals(
				afterReturningAspect.failureCount + afterReturningAspect.successCount,
				afterReturningAspect.afterCount);
	}
	...
	/**
	 * Not supported in 2.0! public void testExplicitDeclarePrecedencePreventsFailure() {
	 * TestBean target = new TestBean(); ITestBean itb = (ITestBean) createProxy(target,
	 * getFixture().getAdvisors(new SingletonMetadataAwareAspectInstanceFactory(new
	 * DeclarePrecedenceShouldSucceed(), "someBean")), ITestBean.class); assertEquals(666,
	 * itb.getAge()); }
	 */
	...
	private class PerTypeWithinAspectInstanceFactory
	...
	public static class ManyValuedArgs {
		...
		public String reverseAdvice(ProceedingJoinPoint pjp, int b, int c, String d,
				StringBuffer e, String a) throws Throwable {
...
/**
 * Add a DeclareParents field in concrete subclasses, to identify the type pattern to
 * apply the introduction to.
 *
 * @author Rod Johnson
 * @since 2.0
 */
...
abstract class AbstractMakeModifiable {
	...
	public void recordModificationIfSetterArgumentDiffersFromOldValue(JoinPoint jp,
		...
		/*
		 * We use the mixin to check and, if necessary, change, modification status. We
		 * need the JoinPoint to get the setter method. We use newValue for comparison. We
		 * try to invoke the getter if possible.
		 */
		...
		Method correspondingGetter = getGetterFromSetter(
...
class MakeITestBeanModifiable extends AbstractMakeModifiable {
	...
	@DeclareParents(value = "org.springframework.tests.sample.beans.ITestBean+", defaultImpl = ModifiableImpl.class)
...
class MakeAnnotatedTypeModifiable extends AbstractMakeModifiable {
	...
	@DeclareParents(value = "(@org.springframework.aop.aspectj.annotation.Measured *)",
			// @DeclareParents(value = "(@Measured *)", // this would be a nice
			// alternative...
...
class MakeLockable {
	...
	public void checkNotLocked(Lockable mixin) // Bind to arg
	{
